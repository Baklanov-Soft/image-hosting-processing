...
trait KafkaJsonDeserializer {
  ...
  implicit def deserializer[F[_]: Sync, T: Decoder]: Deserializer[F, T] =
    Deserializer.instance { (_, _, bytes) =>
      Sync[F].delay(new String(bytes)).flatMap { str =>
        parser
          .parse(str)
          .flatMap(Decoder[T].decodeJson) match {
          case Left(value)  =>
            KafkaJsonDecodingError(s"Can't decode json kafka message: ${value.getMessage}. Original message: $str")
              .raiseError[F, T]
